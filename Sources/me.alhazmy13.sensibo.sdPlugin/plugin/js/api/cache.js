...
function Cache(context = null) {
    ...
    function refreshApiKey(pairedApiKey, apiKey) {
        ...
        if ('data' in globalSettings) {
            instance.data = globalSettings.data;
        }
        log(globalSettings.data);
        // Create api cache
        let keyCache = {acs: {}};
        keyCache.key = api.getKey();
        let oldCache = Object.assign({}, instance.data);
        // let newCache = Object.assign({}, instance.data);
    ...
    function saveCacheData() {
        ...
        if (globalSettings.data === undefined) {
            globalSettings.data = {};
        }

        // Add new key to the global settings
        globalSettings.data = instance.data;
        saveGlobalSettings(instance.context);
