...
function Swing(inContext, inLanguage, inStreamDeckVersion, inPluginVersion) {
    // Init Temperature
    let instance = this;
    ...
    this.localize = () => {
        ...
        if ('swing_mode' in settings) {
            if (settings.swing_mode === 'set') {
                document.getElementById('set').checked = true;
                this.loadExtraPlaceHolder(settings.swing_mode)
            } else {
    ...
    this.loadExtraPlaceHolder = (value) => {
        ...
        if (tempType) {
            ...
            document.getElementById('swing-state-select').value = settings.swing_state;
        } else {
    ...
    function fanLevelValueChanged(inEvent) {
        settings.swing_state = inEvent.target.value;
        instance.saveSettings();
        ...
        instance.sendToPlugin({
            sensiboEvent: 'valueChanged',
            state: {
                swing_state: inEvent.target.value,
